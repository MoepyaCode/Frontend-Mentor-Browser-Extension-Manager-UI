...
function Extension(props: ExtensionI) {
    ...
    return (
        <div className='bg-[#FBFDFE] p-5 rounded-[20px] shadow-[0_0_5px_rgba(0,0,0,.1)] max-w-86.5 min-h-50 flex flex-col justify-between'>
            <div className='flex gap-4'>
                <img className='w-15 aspect-square self-start' src={logo} alt="" />

                <div className='flex flex-col gap-2'>
                    <h2 className='text-[#091540] font-bold text-[20px] leading-[120%] tracking-[-.2px]'>{name}</h2>

                    <p className='text-[#535868] text-[16px] leading-[140%] tracking-[-.5px]'>{description}</p>
                </div>
            </div>

            <div className='flex items-center justify-between '>
                <button onClick={onClickRemove} className='px-4 py-2 text-[#091540] border-2 border-solid border-[#C6C6C6] rounded-full cursor-pointer'>
                    Remove
                </button>
